...
class Board():
    ...
    def __init__(self, colors=Colors(), density=0.2, ui=True, width = 15, height = 15, width_mul = 20, height_mul = 20, rng=None):
        self.colors = colors
        ...
        if (rng == None):
            rng = random.Random()
        self.automata = cellular_automata.init_board(density, self.width, self.height, rng)
        self.ui = ui
        if self.ui:
